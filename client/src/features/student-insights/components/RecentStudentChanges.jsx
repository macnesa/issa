...
function formatRecordDate(occurredAt) {
  ...
  return Number.isNaN(recordDate.getTime()) ? 'Tanggal tidak tersedia' : dateFormatter.format(recordDate);
}
...
function getKkmContext(change) {
  ...
  if (currentValue > kkm) return `Nilai masih berada di atas KKM ${formatScoreValue(kkm)}.`;
  if (currentValue === kkm) return `Nilai terbaru memenuhi KKM ${formatScoreValue(kkm)}.`;
...
function RecentChangeItem({ change, index }) {
  ...
  return (
    <li className={`recent-change recent-change--${domain}`}>
      <span className="recent-change__index" aria-hidden="true">
        {String(index + 1).padStart(2, '0')}
      </span>
      <span className="recent-change__marker" aria-hidden="true" />
      <article>
        <header>
          <span className="recent-change__domain">{domainLabels[domain]}</span>
          <time dateTime={change.occurredAt || undefined}>{formatRecordDate(change.occurredAt)}</time>
        </header>
        <p className="recent-change__title">{content.title}</p>
        {content.detail && (
          change.type === 'feedback'
            ? <blockquote className="recent-feedback-quote">{content.detail}</blockquote>
            : <strong className="recent-change__detail">{content.detail}</strong>
        )}
        {content.context && <small>{content.context}</small>}
      </article>
    </li>
  );
}
...
export default function RecentStudentChanges({ studentId, refreshKey = 0 }) {
  ...
  const loadStudentInsights = useCallback(async () => {
    ...
    setError(null);
    try {
      const { data: studentInsights } = await apiClient.get(`/students/${studentId}/insights`);
      setInsights(studentInsights && typeof studentInsights === 'object' ? studentInsights : null);
      setStatus('success');
    } catch (apiError) {
  ...
  const attendanceRateLabel = recordedDays > 0 && Number.isFinite(attendanceRate)
    ...
    : '—';
  const academicTrendLabel = academicTrendLabels[insights?.academics?.overallTrend] || '—';

  return (
    <section
      className="surface surface--full recent-changes"
    ...
    >
      <SectionHeader
        ...
        description="Ringkasan data faktual, pengukuran akademik, dan observasi guru."
        titleId="recent-student-changes-title"
      />
      ...
      {status === 'success' && (
        <>
          <dl className="recent-changes__metrics">
            <div>
