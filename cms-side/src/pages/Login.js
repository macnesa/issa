...
export default function Login() {
  ...
  const changeInputHandler = (event) => setForm({ ...form, [event.target.name]: event.target.value });

  const submitForm = (event) => {
    event.preventDefault();
    setMessage("");
    setSubmitting(true);
    fetch(`${baseUrl}/teachers/login`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(form) })
      .then(async (response) => {
        const data = await response.json();
        if (!response.ok) throw new Error(data.msg || "Login gagal.");
        return data;
      })
      .then((data) => { localStorage.setItem("access_token", data.access_token); navigate("/"); })
    ...
    <main className="flex min-h-screen items-center justify-center bg-[var(--page)] px-4 py-8">
      <section className="w-full max-w-md rounded-[var(--radius)] border border-[var(--border)] bg-[var(--surface)] p-6 shadow-[var(--shadow)] sm:p-8">
        ...
        <form onSubmit={submitForm} className="space-y-5">
          <label className="block"><span className="mb-1.5 block text-sm font-medium text-[var(--text)]">NIP</span><input required autoComplete="username" type="text" name="NIP" value={form.NIP} onChange={changeInputHandler} className={inputClassName} placeholder="Masukkan NIP" /></label>
          <label className="block"><span className="mb-1.5 block text-sm font-medium text-[var(--text)]">Password</span><input required autoComplete="current-password" type="password" name="password" value={form.password} onChange={changeInputHandler} className={inputClassName} placeholder="Masukkan password" /></label>
