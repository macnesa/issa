  ...
  return (
    <aside className="sticky top-0 z-20 w-full border-b border-slate-800 bg-slate-950 text-slate-100 md:min-h-screen md:w-64 md:border-b-0 md:border-r">
        ...
        <ul className="flex min-w-max gap-1 md:block md:space-y-1">
          {navigation.map((navigationItem) => (
            <li key={navigationItem.to}>
              <NavLink end={navigationItem.end} to={navigationItem.to} className={({ isActive }) => `flex min-h-10 items-center gap-3 rounded-lg px-3 py-2 text-sm font-medium transition ${isActive ? "bg-blue-600 text-white" : "text-slate-300 hover:bg-slate-800 hover:text-white"}`}>
      ...
      <div className="hidden border-t border-slate-800 px-5 py-5 md:block">
        ...
        <button type="button" onClick={handleTeacherLogout} className="mt-5 flex min-h-10 w-full items-center justify-center rounded-lg border border-slate-700 px-3 py-2 text-sm font-semibold text-slate-200 hover:bg-slate-800">Keluar</button>
      </div>
    </aside>
  );
}
